...
logging.basicConfig(filename=".file_sorter.log",
                    level=logging.DEBUG,
...
DEFAULT_EXTENSIONS = {
    ...
    'Images': {
        'exts': ('.png', '.svg', '.webp', '.jpeg', '.jpg', '.gif', '.bmp', '.tif', '.tiff', '.raw', '.ico', '.jfif', '.heic'),
        'directory_name': 'images'
    },
    'Archives': {
        'exts': ('.xz', '.bz', '.tar', '.zip', '.rar', '.7z', '.gz', '.lzip', '.cab', '.iso', '.dmg'),
    ...
    'Videos': {
        'exts': ('.mp4', '.mov', '.avi', '.mkv', '.wmv', '.mpg', '.mpeg', '.m4v', '.webm', '.flv', '.3gp', '.rmvb'),
        'directory_name': 'videos'
    },
    'Audios': {
        'exts': ('.mp3', '.wav', '.ogg', '.flac', '.aif', '.mid', '.midi', '.wma', '.aac', '.m4a', '.opus', '.ape', '.dsd'),
        'directory_name': 'audios'
    },
    'Java': {
        'exts': ('.java', '.jar', '.class', '.javadoc'),
        'directory_name': 'java'
    },
    'Scripts': {
        'exts': ('.sh', '.bash', '.bat', '.fish', '.zsh', '.ps', '.py', '.rb', '.pl', '.lua', '.jsp'),
    ...
    'Configuration': {
        'exts': ('.cfg', '.conf', '.properties', '.ini', '.yaml', '.toml', '.yml', '.config'),
            ...
            for filepath in self._get_file_paths():
                ...
                if extension in ext_data['exts'] or '*' in ext_data['exts']:
                    ...
                    try:
                        ...
                        logging.info(f'[{name}] {path.name} -> {"/".join(new_path.parts[-2:])}')
                        path.rename(new_path)
                    except Exception as ex:
                        logging.error(f'Error raised when working with {self.path}: {ex}')
...
def main():
    ...
    parser.add_argument('--dir', type=str, help='Input dir for sorting', default=f'{os.getenv("HOME")}/Downloads')
